...
MAX_PERCENT_SUMMARY = 50 # 10 # 10 in paper, 34 as discussed

summary_division = {'Introduction':7, 'Context':13, 'Analysis':22, 'Conclusion':6, 'Citation': 2}
...
def LetSum(file,no):
	...
	for line in t:
		...
		#print(line)
		#print(score)
		s=score['Introduction']+score['Context']+score['Analysis']+score['Conclusion']+score['Citation']
		#print(s)
		sum1.append(j)
		sum1.append(line)
		sum1.append(s)
	...
	for category in category_txt:
		...
		words_in_summary = int( (MAX_LENGTH_SUMMARY * length*summary_division[category]) / (100 * 100) )
	...
	# with open('dict_'+str(no)+'.csv', 'w') as csv_file:  
	# 		writer = csv.writer(csv_file)
	# 		#for key, value in sum1.items():
	# 		for i in range(len(sum2)):
	# 			writer.writerow([sum2[i][0],sum2[i][1],sum2[i][2]])
	# 		#writer.writerow([key,value])

	summary_txt = '\n'.join(summary[category] for category in summary)
	with open('./summary/summary_letsum'+str(no)+'.txt','w') as f:
