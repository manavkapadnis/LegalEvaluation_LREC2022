...
MAX_LENGTH_SUMMARY = 500 # Define maxmimum words in summary
...
def get_summary(file,no, summary_fraction):
	'''
	...
	# with open('dictg_'+str(no)+'.csv', 'w') as csv_file:  
	# 	writer = csv.writer(csv_file)
	# 	for key, value in kmm.items():
	# 		writer.writerow([key, value])
	
	# generate_summary
	visited = {}
	summary = {}
	# print(doc_length * summary_fraction)
	for pair in kmix_sorted:
		...
		# print(length)
		if length > doc_length * summary_fraction:
			break
	...
	# print(len(summary_txt))
	save_dir = "../../summary_outputs/Graphical_Model/summary_{}".format(summary_fraction)
	if os.path.exists(save_dir) == False:
		os.mkdir(save_dir)
	with open(os.path.join(save_dir, 'summary_graphical_'+str(no)+'.txt'),'w') as f:
